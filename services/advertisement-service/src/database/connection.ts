...
const pool = new Pool({
    connectionString: process.env.DATABASE_URL || 'postgres://postgres:postgres@localhost:5432/advertisement_db',
    max: 20,
...
export class DatabaseConnection {
    ...
    public async initializeDatabase(): Promise<void> {
        try {
            ...
            // If running from build directory, try the source directory
            ...
            if (!fs.existsSync(schemaPath)) {
                console.log('Schema file not found, creating tables programmatically...');
    ...
    private async createTablesDirectly(): Promise<void> {
        ...
        // Create triggers separately to handle potential conflicts
        const triggers = [
            `CREATE OR REPLACE TRIGGER update_campaigns_updated_at BEFORE UPDATE ON campaigns
                FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();`,
            `CREATE OR REPLACE TRIGGER update_ad_groups_updated_at BEFORE UPDATE ON ad_groups
                FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();`,
            `CREATE OR REPLACE TRIGGER update_advertisements_updated_at BEFORE UPDATE ON advertisements
                FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();`,
            `CREATE OR REPLACE TRIGGER update_ad_transactions_updated_at BEFORE UPDATE ON ad_transactions
                FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();`,
            `CREATE OR REPLACE TRIGGER update_advertisers_updated_at BEFORE UPDATE ON advertisers
                FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();`
        ];
        ...
        for (const trigger of triggers) {
            try {
                ...
                console.log(`Trigger creation skipped (may already exist): ${error}`);
            }
        }

        console.log('Database tables created programmatically');
    }
